...
SELECT EMP_NAME AS "이름", SALARY*12 AS "연봉", (SALARY+SALARY*BONUS)*12 AS "총수령액(보너스포함)",
((SALARY+SALARY*BONUS)*12)-((SALARY+SALARY*BONUS)*12*0.03) AS "실수령액"
FROM EMPLOYEE;
--NULL값 수정 아직
...
SELECT EMP_NAME AS "이름", SALARY AS "월급", ((SALARY+SALARY*BONUS)*12)-((SALARY+SALARY*BONUS)*12*0.03) AS "실수령액" 
, HIRE_DATE AS "고용일"
FROM EMPLOYEE
WHERE ((SALARY+SALARY*BONUS)*12)-((SALARY+SALARY*BONUS)*12*0.03)>=50000000;
--마찬가지고 NULL처리 아직 안됨
...
SELECT EMP_NAME, TO_CHAR(SALARY, '999,999,999') || '원' AS "급여" 
FROM EMPLOYEE;
...
--SYSDATE - HIRE_DATE
SELECT EMP_NAME, DEPT_CODE, SUBSTR(EMP_NO, 1,6),
     MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(HIRE_DATE,1,6), 'YYMMDD'))
FROM EMPLOYEE;
...
SELECT SUM((SALARY+SALARY*BONUS)*12) AS "보너스 포함 연봉 합"
FROM EMPLOYEE
WHERE DEPT_CODE = 'D5';
