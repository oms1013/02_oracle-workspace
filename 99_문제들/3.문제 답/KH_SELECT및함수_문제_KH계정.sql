...
SELECT EMP_NAME, SALARY, (SALARY + SALARY * BONUS) * 12 - (SALARY * 12 * 0.3) AS "실수령액", HIRE_DATE
FROM EMPLOYEE
...
SELECT EMP_NAME, DEPT_CODE,
       ...
       TRUNC(MONTHS_BETWEEN(SYSDATE, TO_DATE(SUBSTR(EMP_NO, 1, 6))) / 12) AS "나이(만)"
FROM EMPLOYEE;
...
SELECT EMPLOYEE.*, DECODE(DEPT_CODE, 'D5', '총무부', 'D6', '기획부', 'D9', '영업부') AS "부서"
FROM EMPLOYEE
WHERE DEPT_CODE IN ('D5', 'D6', 'D9')
ORDER BY DEPT_CODE;
...
SELECT COUNT(*) AS "전체 직원 수",
       COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), '2001', 1)) AS "2001년 입사 인원 수",
       COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), '2002', 1)) AS "2002년 입사 인원 수",
       COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), '2003', 1)) AS "2003년 입사 인원 수",
       COUNT(DECODE(EXTRACT(YEAR FROM HIRE_DATE), '2004', 1)) AS "2004년 입사 인원 수"
FROM EMPLOYEE;