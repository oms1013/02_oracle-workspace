...
SELECT ENAME,
       EXTRACT(YEAR FROM HIREDATE) || '년 ' || EXTRACT(MONTH FROM HIREDATE) || '월' AS "입사일"
FROM EMP;
...
SELECT EMPNO, ENAME, HIREDATE,
       ADD_MONTHS(HIREDATE, 480) AS "입사일로부터 40년 후"
FROM EMP;

--18. EMP테이블에서 입사일로부터 38년 이상 근무한 직원의 정보 조회
SELECT * FROM EMP
WHERE ADD_MONTHS(SYSDATE, -(12 * 38)) >= HIREDATE;

SELECT * FROM EMP
WHERE SYSDATE >= ADD_MONTHS(HIREDATE, 12 * 38);
